...
class ProductRouting {
    static getHomeHtml(homeHtml , products) {
        ...
        products.map((product, index) => {
            tbody +=`
                    ...
                    <td><a href="/product/edit/${product.id}"><button class="btn btn-primary">Sua</button></a></td>
                    <td><a href="/product/delete/${product.id}"><button class="btn btn-danger">Xoa</button></a></td>
                    </tr>
                    `
        });
        ...
        if (req.method === 'GET') {
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    const data = await productService.save(product);
                    res.writeHead(301, {'location': '/home/product/upLoad/${data.insertId}'});
                    res.end();
                }
            })
        }
    ...
    static showFormUpLoad(req, res, id) {
        if (req.method === 'GET') {
            fs.readFile('./views/upLoadFile.html', 'utf-8', (err, upLoadHtml) => {
                if (err) {
                    console.log(err)
                } else {
                    res.writeHead(200, 'text/html');
